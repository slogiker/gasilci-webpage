...
switch ($method) {
    case 'GET':
        if ($id) {
            ...
            $stmt = $db->query("SELECT * FROM vehicles ORDER BY year DESC");
            $items = $stmt->fetchAll();
    ...
    case 'POST':
        ...
        $stmt = $db->prepare("INSERT INTO vehicles (name, description, image, year) VALUES (?, ?, ?, ?)");
        $stmt->execute([
            $data['name'] ?? '',
            $data['description'] ?? '',
            $data['image'] ?? null,
    ...
    case 'PUT':
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        $stmt = $db->prepare("UPDATE vehicles SET name = ?, description = ?, image = ?, year = ? WHERE id = ?");
        $stmt->execute([
            $data['name'] ?? '',
            $data['description'] ?? '',
            $data['image'] ?? null,
